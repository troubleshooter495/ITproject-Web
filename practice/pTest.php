	 ...
	 $('.ans').click(function(){
		...
		if(!anspressed){
	 	if(ints[r_int]==id && flag==0){
	 		outwrite('Верно, правильный ответ - ' + (ints[r_int]).toString() + '. Нажмите кнопку, чтобы услышать следующий звук');
	 	}
	 	else {
	 		outwrite('Неверно, правильный ответ - ' + (ints[r_int]).toString() + '. Нажмите кнопку, чтобы услышать следующий звук');
	 	}
	 	flag=1;
		anspressed = true;
		}
